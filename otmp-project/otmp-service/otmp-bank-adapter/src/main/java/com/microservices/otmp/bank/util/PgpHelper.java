...
//			throw new PGPException("Encrypted message contains a signed message - not literal data.");
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void decryptFile(InputStream in, InputStream keyIn, char[] passwd, List<String> fileDataList)
			throws Exception {
		Security.addProvider(new BouncyCastleProvider());
		in = PGPUtil.getDecoderStream(in);
		PGPObjectFactory pgpF = new PGPObjectFactory(in);
	...
	// 加密流
	public void encryptFile(OutputStream out,InputStream in, String fileName, long length, Date modificationTime,
		...
		ByteArrayOutputStream bOut = new ByteArrayOutputStream();
		PGPCompressedDataGenerator comData = new PGPCompressedDataGenerator(PGPCompressedData.ZIP);
		PGPUtilHelper.writeFileToLiteralData(comData.open(bOut), PGPLiteralData.BINARY,in, fileName, length, modificationTime);
		comData.close();
		JcePGPDataEncryptorBuilder c = new JcePGPDataEncryptorBuilder(PGPEncryptedData.CAST5)
				.setWithIntegrityPacket(withIntegrityCheck).setSecureRandom(new SecureRandom()).setProvider("BC");
